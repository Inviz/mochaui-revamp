...
ART.Container = new Class({
	...
	initialize: function(options, component){
		if(!this.component) this.component = ' art-'+component || 'art-container';
		this.setOptions(options);
		options = this.options;
		
		var absZero = {position: 'absolute', top: 0, left: 0};
		
		this.container = new Element('div', {'class': (this.component!='art-container' ? 'art-container ' : '')+this.component}).setStyles({
			position: options.styles.position, top: options.top, left: options.left
		});
		...
		if(!options.canvas['class']) options.canvas['class'] = options.canvas.className || this.component+'-canvas';
		this.paint = new ART.Paint(options.canvas).setStyles(absZero).inject(this.container);
		
		this.wrapper = new Element('div').setStyles(absZero).inject(this.container);

		this.top = $pick(this.top, {offsetHeight: 0});
		this.bottom = $pick(this.bottom, {offsetHeight: 0});
	...
	process: function(name, part, container){
		...
		if(this[name] && $type(this[name])=='element'){
			where = this[name].inject(container);
			where.getChildren().dispose(); // It is not the task of ART.Container to destroy elements, just to remove them
		}else{
	...
	draw: function(theme){
		if($type(theme)=='string') theme = this.options.theme[theme];
